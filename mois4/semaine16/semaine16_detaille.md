...
## Jour 2 — Drivers Asynchrones & Multithreading
**Cours (2h)** : Driver Java asynchrone, `CompletionStage`, `CompletableFuture`, requêtes préparées et session partagée.
**TP (4h)** : Créer un `ClearingRepository` asynchrone qui persiste les transactions et les positions sans bloquer le pool de threads principal.

## Jour 3 — Pipeline Complet (Kafka → Cassandra)
...
**TP (4h)** : Le moteur v3.1 est le pipeline complet : les virements entrent par Kafka, sont compensés et les résultats sont archivés dans Cassandra. Test de résilience : couper Cassandra, observer la reprise. **Démonstration au tuteur.**
