...
function QuoteLine({ y = 430, size = 62, from = 0, hlAt = null }) {
  ...
  const parts = isFr()
    ? [
        { txt: 'Une ' }, { txt: 'monade,', c: MV.mnd, k: 0 },
        { txt: " c'est simplement un " },
        { txt: 'monoïde', c: MV.mon, k: 3 },
        { txt: ' dans la ' },
        { txt: 'catégorie', c: MV.cat, k: 2 },
        { txt: ' des ' },
        { txt: 'endofoncteurs.', c: MV.fun, k: 1 },
      ]
    : [
  ...
  return (
    <div style={{
      ...
      letterSpacing: isFr() ? 0 : '0.04em', lineHeight: isFr() ? 1.35 : 1.6,
    }}>
      ...
      <span style={{ color: MV.faint, fontWeight: 400, marginLeft: isFr() ? 0 : 6 }}>」</span>
    </div>
  );
}
...
function Scene1() {
  const t = useT();
  const roadNodes = [
    { zh: tr('范畴', 'Catégorie'), c: MV.cat, at: 22.0 },
    { zh: tr('函子', 'Foncteur'), c: MV.fun, at: 23.6 },
    { zh: tr('自函子', 'Endofoncteur'), c: MV.fun, at: 25.2 },
    { zh: tr('幺半群', 'Monoïde'), c: MV.mon, at: 26.8 },
    { zh: tr('单子', 'Monade'), c: MV.mnd, at: 28.4 },
  ];
  ...
  return (
    <Sprite start={0} end={38}>
      ...
      <FadeGroup from={14} to={32.5} exit={1.0}>
        {[
          { zh: tr('范畴', 'Catégorie'), en: 'Category', c: MV.cat, x: 560 },
          { zh: tr('（自）函子', '(Endo)foncteur'), en: 'Endofunctor', c: MV.fun, x: 960 },
          { zh: tr('幺半群', 'Monoïde'), en: 'Monoid', c: MV.mon, x: 1360 },
        ].map((d, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ fontFamily: isFr() ? MV.sans : MV.serif, fontSize: isFr() ? 32 : 38, fontWeight: 700, color: d.c }}>{d.zh}</div>
        ...
        {roadNodes.map((n, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              width: isFr() ? (i === 4 ? 154 : 144) : (i === 4 ? 132 : 112),
              height: i === 4 ? 132 : 112, borderRadius: '50%',
              border: `2.5px solid ${n.c}`, background: `${n.c}${i === 4 ? '2e' : '14'}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontFamily: isFr() ? MV.sans : MV.serif,
              fontSize: isFr() ? (i === 4 ? 24 : 21) : (i === 4 ? 34 : 27),
              fontWeight: 700, color: n.c,
      ...
      <Cap from={1.2} to={6.6} fr="Dans le monde de la programmation fonctionnelle, une phrase tristement célèbre circule.">在函数式编程的世界里，流传着一句臭名昭著的话。</Cap>
      <Cap from={6.6} to={13.4} fr={<>On dit qu'elle peut faire reculer l'ingénieur le plus sûr de lui en trois secondes. <Em c={MV.mnd}>Monade</Em> ? <Em c={MV.fun}>Endofoncteur</Em> ? <Em c={MV.mon}>Monoïde</Em> ?</>}>据说它能在三秒之内，劝退最自信的工程师。<Em c={MV.mnd}>单子</Em>？<Em c={MV.fun}>自函子</Em>？<Em c={MV.mon}>幺半群</Em>？</Cap>
      <Cap from={13.4} to={21.2} fr={<>Mais regardons de près : cette phrase ne contient en réalité que <Em>trois</Em> notions vraiment étrangères.</>}>但仔细看——这句话里，真正陌生的概念其实只有<Em>三个</Em>。</Cap>
      <Cap from={21.2} to={31.8} fr="Nous allons suivre cette route et les ouvrir une par une. Chacune existe déjà dans votre Scala quotidien.">我们沿着这条路，一个一个拆开。每一个，你其实都在 Scala 里天天打交道。</Cap>
      <Cap from={31.8} to={37.4} fr={<>Commençons par la fondation : qu'est-ce qu'une <Em c={MV.cat}>catégorie</Em> ?</>}>先从最底下的地基说起：什么是<Em c={MV.cat}>范畴</Em>？</Cap>
    </Sprite>
  );
}
...
function Scene2() {
  ...
  return (
    <Sprite start={38} end={104}>
      <Chapter from={38.5} to={103} num="Chapter 01" zh="范畴" fr="Catégorie" en="Category" color={MV.cat} />
      ...
      <FadeGroup from={96} to={103.2} exit={0.8}>
        <div style={{
          ...
          {tr('对象', 'Objets')} <span style={{ color: MV.faint }}>+</span> {tr('箭头', 'flèches')} <span style={{ color: MV.faint }}>+</span> {tr('复合', 'composition')} <span style={{ color: MV.faint }}>+</span> {tr('恒等', 'identités')}
          <span style={{ color: MV.faint, margin: '0 16px' }}>⇒</span>
          <span style={{ color: MV.cat }}>{tr('一个范畴', 'une catégorie')}</span>
        </div>
      </FadeGroup>

      <Cap from={39.5} to={46.8} fr={<>Voyez chaque type Scala comme un point. En théorie des catégories, ces points sont des <Em c={MV.cat}>objets</Em>.</>}>把每一个 Scala 类型看成一个点。范畴论管这些点叫——<Em c={MV.cat}>对象</Em>。</Cap>
      <Cap from={46.8} to={54.8} fr={<>Une fonction entre deux types devient une <Em c={MV.cat}>flèche</Em> entre deux points. Par exemple <Em>String =&gt; Int</Em>.</>}>类型之间的函数，就是点与点之间的<Em c={MV.cat}>箭头</Em>。比如 <Em>String =&gt; Int</Em>。</Cap>
      <Cap from={54.8} to={63.6} fr={<>Ajoutons <Em>Int =&gt; Boolean</Em>. Les flèches commencent à se multiplier.</>}>再来一条 <Em>Int =&gt; Boolean</Em>。箭头开始多起来了。</Cap>
      <Cap from={63.6} to={73.2} fr={<>Deux flèches mises bout à bout se <Em c={MV.cat}>composent</Em> en une nouvelle flèche : c'est le <Em>andThen</Em> que vous avez écrit mille fois.</>}>首尾相接的箭头可以<Em c={MV.cat}>复合</Em>出一条新箭头——这就是你写过无数次的 <Em>andThen</Em>。</Cap>
      <Cap from={73.2} to={82.8} fr={<>Chaque objet possède aussi une flèche qui revient sur lui-même : <Em>identity</Em>. Elle ne fait rien, mais elle est indispensable.</>}>每个对象还自带一条原地打转的箭头：<Em>identity</Em>。它什么都不做，却不可或缺。</Cap>
      <Cap from={82.8} to={93.8} fr={<>Il n'y a que deux règles : la composition est <Em c={MV.cat}>associative</Em>, et identity est l'<Em c={MV.cat}>unité</Em> de la composition.</>}>规则只有两条：复合满足<Em c={MV.cat}>结合律</Em>，identity 是复合的<Em c={MV.cat}>单位</Em>。仅此而已。</Cap>
      <Cap from={93.8} to={103.4} fr={<>Objets, flèches, composition, identités : voilà toute une catégorie. Chaque ligne de Scala vit dans la <Em c={MV.cat}>catégorie des types et fonctions</Em>.</>}>对象、箭头、复合、恒等——这就是范畴的全部。你写的每一行 Scala，都活在<Em c={MV.cat}>类型与函数构成的范畴</Em>里。</Cap>
    </Sprite>
  );
}
